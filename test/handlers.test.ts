 ...
 */
import { serveDeckJson, renderPlayerHtml } from '../src/server/handlers.src'
...
describe('renderPlayerHtml (direct UI page — the only HTML surface)', () => {
    ...
    test('creator-supplied content cannot break out of the script context', () => {
        ...
        const html = renderPlayerHtml('svc.display.sd-room1')
        const injected = /window\.__ODM_DECK__ = (\{.*?\});/.exec(html)
...
describe('session token (the deck poll carries X-UserToken)', () => {
    test('deck JSON carries the session token so the shell can poll authenticated', () => {
        asCreator()
        const res = fakeResponse()
        serveDeckJson(fakeRequest({}, { screen: 'svc.display.sd-room1' }), res)
        expect(JSON.parse(res.state.body).token).toBe('fake-session-token-123')
    })
    test('rendered HTML carries the token too (injected server-side)', () => {
        asCreator()
        expect(renderPlayerHtml('svc.display.sd-room1')).toContain('"token":"fake-session-token-123"')
    })
})
