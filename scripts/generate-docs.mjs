/**
 * generate-docs.mjs — derive DOCUMENTATION.md (and README fact block) from source.
 *
...
const prevDoc = existsSync(join(root, 'DOCUMENTATION.md')) ? read('DOCUMENTATION.md') : ''

const doc = [
    `# ${nowConfig.name} — Technical Documentation`,
    '',
    ...
    '## Data model',
    '',
    generatedBlock('tables', renderTables(byKind('Table'))),
    '',
    '## REST API',
    '',
    generatedBlock('rest', renderRest(byKind('RestApi'), scope)),
    '',
    '## Business rules',
    '',
    ...
    '## Access control (ACLs)',
    '',
    generatedBlock('acls', renderAcls(byKind('Acl'))),
    '',
    '## UI policies',
    '',
    ...
    '## File inventory',
    '',
    generatedBlock('inventory', renderInventory()),
    '',
].join('\n')

writeFileSync(join(root, 'DOCUMENTATION.md'), doc.replace(/\n{3,}/g, '\n\n').trimEnd() + '\n')
...
console.log(`generate-docs: DOCUMENTATION.md + README.md written (${artifacts.length} Fluent artifacts).`)
